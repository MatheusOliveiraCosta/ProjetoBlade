...
public class DetalhesManutencaoTela extends JDialog {
    ...
    public DetalhesManutencaoTela(JFrame parent, Manutencao manutencao) {
        this(parent, manutencao, null);
    }
    
    public DetalhesManutencaoTela(JFrame parent, Manutencao manutencao, com.mycompany.projetoblade.service.ManutencaoService manutencaoService) {
    ...
    private void salvarAlteracoes() {
        try {
            ...
            // 4. Salva no repositório usando o serviço
            if (manutencaoService != null) {
                manutencaoService.salvarManutencao(manutencao);
            }
            
            // 5. Feedback e Fechar
            JOptionPane.showMessageDialog(this, "Manutenção atualizada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
            dispose();
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Erro ao atualizar manutenção: " + ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
