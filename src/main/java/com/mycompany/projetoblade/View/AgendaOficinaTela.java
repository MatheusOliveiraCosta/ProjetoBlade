...
public class AgendaOficinaTela extends JDialog {
    ...
    private void abrirDetalhesManutencao(int row) {
        ...
        try {
            DetalhesManutencaoTela detalhes = new DetalhesManutencaoTela((JFrame) SwingUtilities.getWindowAncestor(this), manutencaoSelecionada, manutencaoService);
            detalhes.setVisible(true);
            // Recarrega a lista após fechar
            carregarDadosDaService();
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Erro ao abrir detalhes: " + ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
